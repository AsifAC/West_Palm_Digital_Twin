...
public class carAI_version2 : Agent
{
...
*/
    ...
    {
/*        randomSpawnLocation = (int)Random.Range(0, 2f);

        if(randomSpawnLocation == 1f)
        {
            transform.position = spawnC.position;
            //transform.position = spawnC.position;
        } else
        {
            transform.position = spawnD.position;
            //transform.position = spawnD.position;
...
/*        if (other.CompareTag("Goal") && (other.gameObject.transform == targetTransform))
        {
...
/*        if (other.CompareTag("Wall"))
        ...
        /*
                        if (other.CompareTag("Red Light"))
                        {
                            ...
                            AddReward(-1f); //used to be 0.5 but kept running it
